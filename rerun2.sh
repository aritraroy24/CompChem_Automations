...
for f in $(find . -name "*.chk"); do
		...
		mkdir ~aritra/gaussian/output/success/$(basename -s .chk $f)_3
                ...
                echo "\n${YELLOW}${filename}_3.gjf${RESTORE}, ${YELLOW}${filename}_3.log${RESTORE} & ${YELLOW}$filename.chk${RESTORE} files have been moved into the ${CYAN}output/success/${filename}_3${RESTORE} folder\n-----------------\n"
        else
		mkdir ~aritra/gaussian/output/error/$(basename -s .chk $f)_3
                ...
                echo "\n${YELLOW}${filename}_3.gjf${RESTORE}, ${YELLOW}${filename}_3.log${RESTORE} & ${YELLOW}$filename.chk${RESTORE} files have been moved into the ${CYAN}output/error/${filename}_3${RESTORE} folder\n-----------------\n"
        fi
	sleep 0.5
done
