...
for f in $(find . -name "*.chk"); do
		...
		mkdir ~aritra/gaussian/output/success/$(basename -s .chk $f)_2
                ...
                mv $(basename -s .chk $f).fchk ~aritra/gaussian/output/success/$(basename -s .chk $f)_2
                echo "\n${YELLOW}${filename}_2.gjf${RESTORE}, ${YELLOW}${filename}_2.log${RESTORE}, ${YELLOW}$filename.chk${RESTORE} & ${YELLOW}$filename.fchk${RESTORE} files have been moved into the ${CYAN}output/success/${filename}_2${RESTORE} folder\n-----------------\n"
        else
		mkdir ~aritra/gaussian/output/error/$(basename -s .chk $f)_2
                ...
                mv $(basename -s .chk $f).fchk ~aritra/gaussian/output/error/$(basename -s .chk $f)_2
                echo "\n${YELLOW}${filename}_2.gjf${RESTORE}, ${YELLOW}${filename}_2.log${RESTORE}, ${YELLOW}$filename.chk${RESTORE} & ${YELLOW}$filename.fchk${RESTORE} files have been moved into the ${CYAN}output/error/${filename}_2${RESTORE} folder\n-----------------\n"
        fi
	sleep 0.5
done
